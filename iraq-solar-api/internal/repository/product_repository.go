...
func (r *postgresProductRepository) Create(ctx context.Context, product *domain.Product) error {
	...
	query := `
		INSERT INTO products (id, category_id, merchant_id, store_id, branch_id, sku, name, brand_id, model, type, price_iqd, stock_quantity, reserved_quantity, low_stock_threshold, specifications, images, is_available, created_at, updated_at)
		VALUES (:id, :category_id, :merchant_id, :store_id, :branch_id, :sku, :name, :brand_id, :model, :type, :price_iqd, :stock_quantity, :reserved_quantity, :low_stock_threshold, :specifications, :images, :is_available, :created_at, :updated_at)
	`
...
func (r *postgresProductRepository) ListAll(ctx context.Context) ([]domain.Product, error) {
	...
	query := `SELECT p.id, p.category_id, p.merchant_id, p.store_id, p.branch_id, p.sku, p.name, p.brand_id, b.name AS brand_name, p.model, p.type, p.price_iqd, p.stock_quantity, p.reserved_quantity, p.low_stock_threshold, p.specifications, p.images, p.is_available, p.created_at, p.updated_at 
                  FROM products p 
